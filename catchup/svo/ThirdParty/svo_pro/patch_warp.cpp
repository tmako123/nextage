    ...
    AffineTransformation2* A_cur_ref)
{
  ...
  A_cur_ref->col(1) = (px_dv_cur - px_cur) / kHalfPatchSize;
        ...
        AffineTransformation2& A_cur_ref)
    {
        ...
        const Eigen::Matrix3d H_cur_ref = T_cur_ref.getRotationMatrix() * (normal_ref.dot(xyz_ref) * Eigen::Matrix3d::Identity() - t * normal_ref.transpose());
        ...
        uint8_t* patch)
    {
        ...
        for (int y = 0; y < patch_size; ++y) {
            uint8_t* img_ptr = (uint8_t*)img.data
                ...
                + (u_r - halfpatch_size);
            for (int x = 0; x < patch_size; ++x, ++patch_ptr, ++img_ptr) {
                *patch_ptr = wTL * img_ptr[0] + wTR * img_ptr[1] + wBL * img_ptr[step] + wBR * img_ptr[step + 1];
                ;
            }
        }
    }
