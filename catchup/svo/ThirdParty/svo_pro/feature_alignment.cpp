...
#define SVO_DISPLAY_ALIGN_1D 0
        ...
        double* h_inv)
    {
        ...
        alignas(16) float ref_patch_dv[kPatchArea];
        ...
        for (int iter = 0; iter < n_iter; ++iter) {
            ...
            for (int y = 0; y < kPatchSize; ++y) {
                uint8_t* it = (uint8_t*)cur_img.data + (v_r + y - kHalfPatchSize) * cur_step + u_r - kHalfPatchSize;
                for (int x = 0; x < kPatchSize; ++x, ++it, ++it_ref, ++it_ref_dv) {
                    float cur_intensity = wTL * it[0] + wTR * it[1] + wBL * it[cur_step] + wBR * it[cur_step + 1];
        ...
        std::vector<Eigen::Vector2f>* each_step)
    {
        ...
        const int halfpatch_size_ = 8; //4;
        const int patch_size_ = halfpatch_size_ * 2;
        ...
        alignas(16) float ref_patch_dx[patch_area_];
        alignas(16) float ref_patch_dy[patch_area_];
        Eigen::Matrix4f H;
        H.setZero();
...
#define DESCALE(x, n) (((x) + (1 << ((n)-1))) >> (n)) // rounds to closest integer and descales
        ...
        Keypoint& cur_px_estimate)
    {
        ...
        alignas(16) int16_t ref_patch_dx[patch_area];
        alignas(16) int16_t ref_patch_dy[patch_area];
...
#define SVO_DESCALE(x, n) (((x) + (1 << ((n)-1))) >> (n))
        ...
        const int patch_area = 16 * 16;
        if (max_patch_area != patch_area) {
            std::cout << "invalid patch size " << max_patch_area << "/" << patch_area << std::endl;
            return false;
        }
        alignas(16) uint8_t ref_patch[patch_area];
        alignas(16) int16_t ref_patch_dx[patch_area];
        alignas(16) int16_t ref_patch_dy[patch_area];
